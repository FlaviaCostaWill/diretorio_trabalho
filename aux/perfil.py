...
def agg_perfil_geral(df):
    campos_perfil = cps
    chave = ['chave']
    chave2 = campos_perfil + chave
    agg = df[chave2].groupby(campos_perfil, as_index=False).agg({'chave':['count']})
    return agg
...
def agg_perfil_grupos(df, campogrupo):
    ...
    chave2 = campos_perfil + chave + var_grupo
    campo_agg = campos_perfil + var_grupo
    agg = df[chave2].groupby(campo_agg, as_index=False).agg({'chave':['count']})
    return agg
...
def report_perfil_grupos_v1(df2, coluna_grupo, coluna_contagem, dif_var, n_var, colunas_perfil):
    from collections import OrderedDict
    casos_signif_final = pd.DataFrame([], columns=['segmento', 'n_seg_grupo', 'n_seg','n_grupo','total','per_grupo_seg','per_grupo_total','diferenca','diferenca_abs'])
...
#     chave = colunas_perfil.append(coluna_grupo).append(coluna_contagem)
    chave = colunas_perfil
    chave.append(coluna_grupo)
    chave.append(coluna_contagem)
    ...
    for i in list(range(2, len(colunas_perfil))):
        ...
        dfr = pd.concat([dfr, df[[coluna_grupo, coluna_contagem]]], axis = 1) #join to concat
